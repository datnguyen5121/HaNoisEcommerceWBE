...
let createNewProduct = async (req, res) => {
    try {
        const { gender, price, title, productName, description, datePublish, category, size } = req.body;
...
let getProductByGenderCategory = async (req, res) => {
    try {
        ...
        let category = req.query.category;
        const data = await productService.getProductByGenderCategory(category);
        if (data) {
            return res.status(200).json(data);
        } else {
            throw new Error('get product failed!');
